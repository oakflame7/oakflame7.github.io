...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .kick{font-family:"Courier New",monospace;font-size:10.5px;letter-spacing:.24em;
  ...
  #titlebg{position:absolute;inset:0;width:100%;height:100%;image-rendering:pixelated}
  #tmain{position:relative;margin:auto 0;padding:24px 34px;max-height:100%;overflow:auto;
    width:min(440px,94vw);background:linear-gradient(90deg,rgba(5,8,10,.92) 0%,rgba(5,8,10,.62) 74%,rgba(5,8,10,0));
    -webkit-overflow-scrolling:touch}
  #tnav{display:flex;flex-direction:column;gap:7px;align-items:flex-start;margin-top:16px}
  .tbn{background:none;border:none;border-left:2px solid #33423f;color:var(--bone);
    font-family:"Courier New",monospace;font-size:13px;letter-spacing:.22em;text-transform:uppercase;
    padding:8px 0 8px 14px;cursor:pointer;text-align:left;min-width:236px}
  .tbn:hover{border-left-color:var(--brass);color:var(--ink)}
  .tsub{margin:10px 0 6px 16px;max-width:352px}
  .tdf{display:block;width:100%;background:none;border:none;border-left:2px solid #2b3a38;color:#c8d2ce;
    font-family:"Courier New",monospace;font-size:11px;letter-spacing:.18em;text-transform:uppercase;
    text-align:left;padding:6px 0 6px 12px;margin-bottom:6px;cursor:pointer}
  .tdf span{display:block;font-family:"Hoefler Text","Baskerville","Iowan Old Style",Georgia,serif;
    font-size:12.5px;color:#8b9793;text-transform:none;letter-spacing:0;margin-top:2px;line-height:1.45}
  .tdf:hover{border-left-color:var(--brass)}
  .tdf.sel{border-left-color:var(--brass);color:var(--brass)}
  .tbn2{background:none;border:1px solid #2b3a38;color:var(--ash);font-family:"Courier New",monospace;
    font-size:10.5px;letter-spacing:.18em;text-transform:uppercase;padding:8px 12px;cursor:pointer;margin-bottom:10px}
  .tbn2:hover{border-color:var(--brass);color:var(--ink)}
  #tmain .keys{margin:6px 0 4px}
  #mpbox .btn2{margin:0 8px 8px 0}
...
<div id="wrap"><div id="stage">
  ...
  <div class="panel" id="craft">
    <h2>What can be made</h2><div class="sub">Made from what is in your pack</div>
    <div id="reclist"></div>
    <div class="hint">c closes</div>
  </div>

  <div class="panel" id="buildp">
    <h2 id="buildttl">What can be put up</h2><div class="sub" id="buildsub">Click to build it where you stand</div>
  ...
  <div class="panel" id="cachep">
    <h2 id="cacheh2">The hold</h2><div class="sub" id="cachesub">Storage at the craft · click to move a stack</div>
    <div class="two">
  ...
  <div class="panel" id="cityp">
    <h2>Cities</h2><div class="sub">Large, populated, and free of monsters</div>
  ...
  <div class="panel" id="artp">
    <h2>Artwork</h2><div class="sub">The sprites the game draws with</div>
  ...
  <div class="panel" id="regard">
    <h2 id="rgname">No Regard</h2><div class="sub" id="rgsub">No Regard held yet</div>
  ...
  <div class="panel" id="chart">
    <h2>The chart</h2><div class="sub" id="chartsub">Where you can sail from here</div>
  ...
  <div class="panel" id="logp">
    <h2>What has happened</h2><div class="sub">A record of the run</div>
    <div id="loglist"></div>
  ...
  <div class="full on" id="title">
    <canvas id="titlebg" width="504" height="294"></canvas>
    <div id="tmain">
      <h1>Castaway</h1>
      <div class="kick">The Sea of Horrors Exploration Game</div>
      <div id="tnav">
        <button class="tbn" id="start">play</button>
        <button class="tbn" id="contbtn" style="display:none">pick the run back up</button>
        <button class="tbn" id="diffbtn">difficulty · <span id="diffcur">easy</span></button>
        <button class="tbn" id="setbtn">settings</button>
        <button class="tbn" id="mpbtn">multiplayer</button>
      </div>
      <div class="tsub" id="diffbox" style="display:none">
        <button class="tdf" id="df_easy">easy<span>The sea lets you learn. Everything out there hits a little softer.</span></button>
        <button class="tdf" id="df_hard">hard<span>The sea as it has been. A fair fight, if you keep your feet.</span></button>
        <button class="tdf" id="df_canon">canon<span>The sea as the book has it. It does not care whether you make it.</span></button>
      </div>
      <div class="tsub" id="setbox" style="display:none">
        <button class="tbn2" id="sndbtn">sound · on</button>
        <div class="keys">
      ...
      <div class="tsub" id="mpbox" style="display:none">
        <div class="kick" style="margin-bottom:8px">Play together — no server, the two browsers connect directly</div>
        <div id="netui">
          <div id="netbtns">
  ...
  <div class="full" id="ending"><div class="card">
    <div class="kick">The run ends</div>
...
/* ============================================================ DATA: PEOPLE */
const FIRST=["Abner","Amos","Asa","Caleb","Cyrus","Eli","Ezra","Gideon","Hiram","Jasper",
             "Jedediah","Josiah","Levi","Micah","Rufus","Silas","Thaddeus","Warren","Wilbur",
             "Zachariah","Augustus","Clement","Ezekiel","Obadiah","Amasa","Barnabas","Eldridge",
             "Abigail","Adelaide","Clara","Cora","Eliza","Harriet","Hattie","Lavinia","Lucinda",
             "Mabel","Matilda","Mercy","Phoebe","Prudence","Susannah","Tabitha","Winifred",
             "Josephine","Nellie","Pearl","Dorothea","Minerva"];
const LAST =["Whitfield","Barlow","Cartwright","Holloway","Merriweather","Ashford","Bellamy",
             "Crane","Dawson","Eastman","Fairbanks","Granger","Hobbs","Ingram","Jessup","Kimball",
             "Lockwood","Mercer","Newcomb","Osgood","Prescott","Quimby","Rutledge","Sturgis",
             "Thackery","Underwood","Vance","Webb","Yancey","Abernathy","Boone","Ellsworth",
             "Finch","Gaines","Harlan","Ives","Judd","Kirkland","Colfax","Dunmore"];
const TRADES=[
...
let HOME_DRIFT=0;
/* the three seas. Easy softens everything a little; hard is the sea as it was
   tuned; canon is the sea as the book has it. Applied on top of threat. */
const DIFFMUL={
  easy :{hp:.85,dmg:.80,sp:.97,mist:1.0},
  hard :{hp:1,  dmg:1,  sp:1,  mist:1.0},
  canon:{hp:2.6,dmg:1.9,sp:1.12,mist:1.4}
};
function DIFF(){ return DIFFMUL[G.diff]||DIFFMUL.hard; }
function diffGet(){ try{ const d=localStorage.getItem("castaway_diff"); return DIFFMUL[d]?d:"easy"; }catch(e){ return "easy"; } }
const THREATMUL=[
...
function mkFoe(kind,x,y){
  const b=FOES[kind], m=THREATMUL[ISL.threat]||THREATMUL[0], dm=DIFF();
  const scale=1+G.isl*.09;
  const hp=b.hp*m.hp*dm.hp*scale;
  foes.push({x,y,k:kind,n:b.n,hp,mhp:hp,
    sp:b.sp*m.sp*dm.sp,r:b.r,dmg:b.dmg*m.dmg*dm.dmg*(1+G.isl*.04),c:b.c,
    mist:Math.round(b.mist*m.mist*dm.mist*(1+G.isl*.10)),armor:(b.armor||0)+Math.floor(G.isl*.25),
...
function mkBig(tier,x,y){
  ...
  const dm=DIFF(); s.hp*=dm.hp; s.dmg*=dm.dmg; s.sp*=dm.sp; s.mist=Math.round(s.mist*dm.mist);
...
function spawnAnimals(sx,sy){
  const pl=(tutOn()&&G.isl===0)?["deer","deer","squirrel"]:(ANIMAL_POOL[ISL.theme]||["deer"]);
  let n;
...
function populate(sx,sy){
  ...
  if(tutOn()&&G.isl===0){ RULES.n=0; RULES.minis=[]; RULES.bosses=[]; }   // the first island waits for you
  ...
  if(G.isl===2&&!G.regards.length&&!party.some(m=>m&&m.g)) sp=1;   // nobody reaches the third island with nothing
  if(G.regards.length>=MAXREGARDS) sp*=.6;    // and not that generous either
  if(tutOn()&&G.isl<2) sp=-1;                 // the opening places its own spirit
  if(rnd()<sp){
    let x,y;
  ...
  if(tutOn()&&G.isl<2) cn=0;                   // the opening decides who is out here
...
function newIsland(theme,threat){
  ...
  animals=pets;builds=[];
  const tutBig=tutOn()&&G.isl===1;
  resizeWorld(tutBig?100:ISL_W,tutBig?72:ISL_H);
  gates=[];
  let start=null,tries=0,ok=0;
  do{
    newNoise(); layTerrain(theme);
    if(theme==="town") ISL.buildings=layTown();
    else if(rnd()<.45||(tutOn()&&G.isl===1)) ISL.ruin=layRuin(theme);
...
function setSail(theme,threat){
  ...
  if(tutOn()&&G.isl===0){
    G.isl++;
    interlude("The crossing","Toward the fire",
      "You put the raft in and the water takes it, and for once you and the water want the same thing. "+
      "The fire on the far sand grows from a spark to a shape to a place \u2014 and the shapes around it stop "+
      "moving, one by one, while you are still too far out to matter.<br><br>"+
      "<i>By the time your feet find bottom, the beach has gone quiet the way things go quiet after. Not before.</i>",
      ()=>{ newIsland("forest",0); });
    return;
  }
  if(tutOn()&&G.isl===1){
    G.isl++;
    let th=rollTheme(false); if(th==="town") th="outcrop";
    interlude("The crossing","The wind has opinions",
      "He points at a heading and you hold it for an hour, and then the wind comes around hard out of the "+
      "north and holds it longer than you can. The raft goes where it is told \u2014 just not by you.<br><br>"+
      "<i>A coast comes up out of the fog that is on nobody's chart. They never are.</i>",
      ()=>{ newIsland(th,Math.min(1,rollThreat())); });
    return;
  }
...
function objectiveText(){
  ...
  const tl=tutObjLine();
  if(tl){ L.push(lab0("The basics")+"<br>"+tl); if(G.tut===0) return L.join("<br>"); }
  ...
  if(!HOME&&ISL.threat<2&&G.isl>0){
    ...
    L.push(m.length?`${lab("A hearth would claim this island")}<br>Wants ${m.join(", ")}.`
                   :`${lab("A hearth would claim this island")}<br>You have enough. <b>C</b> to lay it.`);
  }
  if(ISL.home){
...
function togglePanel(id){
  ...
  if(was) return;
  $(id).classList.add("on");
  if(id==="pack"){ G.tutPack=1; drawPack(); }
...
function drawRegard(){
  if(!G.regards.length){
    ...
    $("rgsub").textContent="No Regard held yet";
    $("rgrank").innerHTML="";
  ...
  $("rgsub").textContent=`${G.regards.length} of ${MAXREGARDS} carried · ${Math.floor(G.mistTotal)} mist collected in total`;
...
function drawChart(){
  ...
  $("chartsub").textContent=G.craft?"Where you can sail from here":"The coast as you have walked it";
...
function drawCache(){
  ...
  $("cachesub").innerHTML=!any
    ? "You are not standing near any storage"
    : "Click a stack to move it across · "+(atHome
        ? "your island's store — it is safe even if the boat sinks"
        : (ISL&&ISL.city
  ...
  const side=(arr,wch)=>{
    ...
    return h||`<div class="empt">Nothing here</div>`;
  };
...
function talkSpirit(s){
  ...
  if(s.tut&&!s.done){
    ...
    say(s.n,"a spirit",[
      `<i>${s.line}</i>`,
      "It is old the way the sea floor is old, and it has watched you cross its island the way you would watch weather.",
      "<i>It decides about you, all at once \u2014 and being decided about turns out to be a physical event.</i>"
    ],null,()=>grantRegard("the ruins on "+ISL.name));
    return;
  }
  if(s.stage===0){
    const t=s.task;
...
function joinUp(c){
  ...
  syncHUD();
  if(tutOn()&&c.tut&&!G.tutRuins){
    G.tutRuins=1;
    setTimeout(()=>{ if(G.mode!=="play"||interOn) return;
      say(c.n,c.role,[
        `<i>"Before the water, I was three days on this island. There are ruins on the far side. Old. Wrong old."</i>`,
        `<i>"The others talked about something living in them that looks at you \u2014 that gives things, if it likes what it sees. They went to find out, and I heard how that ended. So I ran."</i>`,
        `<i>"You fight like it might go differently for you. It is across the island. I will walk behind."</i>`]);
    },700);
  }
}
function talkCast(c){
...
/* ============================================================ AUDIO */
let AC=null;
const SND={off:(function(){ try{ return localStorage.getItem("castaway_snd")==="off"; }catch(e){ return false; } })()};
...
function fireShot(from,tx,ty,rng){
  ...
  shots.push({x:from.x+Math.cos(a)*(from.r+6),y:from.y+Math.sin(a)*(from.r+6),
    vx:Math.cos(a)*rng.sp,vy:Math.sin(a)*rng.sp,dmg:rng.dmg*DIFF().dmg,r:6,life:2.6,
...
function update(dt){
  ...
  if((G.mode==="play"||(NET.on&&G.mode==="dialog"))&&!halted){
    ...
    for(const c of neutrals.slice()){
      if(c.down){ if(!c.tut){ c.bleed-=dt; if(c.bleed<=0) killAlly(c); } continue; }
...
/* ============================================================ THE OPENING
   Three islands that teach. The first waits for you and shows you your hands;
   then a fire across the water. The second holds a rescue and the first
   Regard. The third suggests a hearth and lets go of your sleeve. All of it
   lives in G, so a saved run picks the lesson back up where it stopped. */
function tutOn(){ return typeof G.tut==="number"&&G.tut<9; }
function packTotal(){ let n=0; for(const s of G.pack) if(s) n+=s.n; return n; }

function tutArrive(){
  if(!tutOn()) return;
  if(G.isl===0){ spawnT=1e9; G.tutStep=0; G.tutW=0; G.tutPack=0; }
  else if(G.isl===1){
    G.tut=2; spawnT=40;
    /* the one who made it, face down where the fight ended */
    const p0=tc(ISL.start[0],ISL.start[1]);
    const c=mkCastaway(p0.x+66,p0.y-24);
    c.down=1; c.bleed=0; c.tut=1; c.asked=0; c.want={k:"free"};
    c.line="You did that for somebody you had no reason to do it for.";
    neutrals.push(c);
    if(!have("bandage")){ give("bandage",1);
      toast("in the wrack","A bandage, of all things, at your feet. The sea keeps strange accounts."); }
    /* something old on the far side, holding what he will speak of.
       layRuin speaks in tiles; convert before comparing or placing. */
    let sx,sy;
    const rq=ISL.ruin?tc(ISL.ruin.x,ISL.ruin.y):null;
    if(rq&&dist(rq.x,rq.y,player.x,player.y)>26*TILE){ sx=rq.x; sy=rq.y; }
    else { const s=farSpot(ISL.start[0],ISL.start[1],30); const q=tc(s[0],s[1]); sx=q.x; sy=q.y; }
    ISL.spirit={x:sx,y:sy,r:13,n:"something the ruins kept",task:{k:"none"},stage:0,
      ph:rnd()*6.3,done:false,tut:1,
      line:"It has been waiting so long that waiting is most of what it is now."};
    /* the way over is not free */
    for(let i=1;i<=6;i++){
      const f=i/7;
      const wx=player.x+(sx-player.x)*(.3+.62*f)+(rnd()*180-90);
      const wy=player.y+(sy-player.y)*(.3+.62*f)+(rnd()*180-90);
      const tx2=clamp(Math.floor(wx/TILE),2,GW-3), ty2=clamp(Math.floor(wy/TILE),2,GH-3);
      if(isReach(tx2,ty2)){ const q=tc(tx2,ty2); if(free(q.x,q.y,12)) mkFoe(pick(pool()),q.x,q.y); }
    }
  }
  else if(G.isl===2){ G.tut=4; }
}
...
function tutObjLine(){
  ...
  if(G.isl===0){
    if(G.tut===0) return [
      th?"Left thumb on the ground \u2014 that is your feet.":"WASD \u2014 that is your feet.",
      "Walk to the wreckage and take it \u2014 "+(th?"the E button":"E")+" beside a piece.",
      (th?"Tap PACK in the strip up top":"Press Tab")+" \u2014 your pack. Close it to go on.",
      (th?"Tap PACK in the strip up top":"Press Tab")+" \u2014 your pack. Close it to go on.",
      th?"Hold ATK \u2014 it keeps swinging. DASH slips a blow.":"Click to strike. Space dashes through a blow.",
      "Two more. Vigor first, then the swing."
    ][G.tutStep]||"";
    if(G.tut===1) return "People, across the water, losing. A raft gets you there.";
  }
  if(G.isl===1){
    if(G.tut===2&&!G.tutRuins) return "Somebody is face-down near the fire. Stand over them \u2014 "+(th?"E":"H or E")+" \u2014 with a bandage.";
    if(G.tut===2) return "The ruins on the far side. Something in them gives things.";
    if(G.tut===3) return "East, he says. He came out of the east.";
  }
  return null;
}

function tutTick(dt){
  if(NET.on){ G.tut=9; return; }
  const th=TC.on;
  if(G.tut===0&&G.isl===0){
    if(G.tutStep===0){
      if(!G.tutT0){ G.tutT0=1; G.tutPX=player.x; G.tutPY=player.y; G.tutM=0;
        say("the sand","",[
          "Sand. Fog. An island behind the fog with something on it. Start with your feet.",
          th?"<i>Left thumb anywhere on the ground \u2014 that is your feet. Walk a little.</i>"
            :"<i>WASD \u2014 that is your feet. Walk a little.</i>"]); }
      G.tutM=(G.tutM||0)+dist(player.x,player.y,G.tutPX,G.tutPY);
      G.tutPX=player.x; G.tutPY=player.y;
      if(G.tutM>130){ G.tutStep=1; G.tutT0=0;
        say("the wreck","",[
          "What the ship was is on the sand around you, in pieces. All of it is worth stooping for.",
          "<i>Walk to a piece and "+(th?"tap E":"press E")+" beside it to take it.</i>",
          "The deer out here are the only things that mean you no harm \u2014 which is unlucky for them. Meat, if you can catch one."]); }
    } else if(G.tutStep===1){
      if(!G.tutT0){ G.tutT0=1; G.tutN=packTotal(); }
      if(packTotal()>G.tutN){ G.tutStep=2; G.tutT0=0;
        say("carried","",[
          "It goes on your back, and your back has slots.",
          "<i>"+(th?"Tap PACK in the strip up top":"Press Tab")+" to see what you hold. Salvage becomes rafts, spears, bandages \u2014 the craft panel turns one into the other. Close the pack to go on.</i>"]); }
    } else if(G.tutStep===2){
      G.tutW+=dt;
      if(G.tutPack||G.tutW>16){ G.tutStep=3; G.tutW=0; }
    } else if(G.tutStep===3){
      if(anyPanel()) return;                    // let them close it themselves
      G.tutStep=4;
      const s=spawnEdge();
      const f=s?mkFoe("hollow",s.x,s.y)
               :mkFoe("hollow",clamp(player.x+9*TILE,TILE,GW*TILE-TILE),player.y);
      say("the sand","",[
        "Something has come out of the treeline, and it is not walking the way a person walks.",
        th?"<i>Hold ATK and you keep swinging \u2014 it aims itself at the nearest thing. DASH slips through a blow, and costs vigor.</i>"
          :"<i>Aim with the mouse. Click to strike \u2014 every swing costs vigor. Space dashes through a blow when it comes.</i>"]);
    } else if(G.tutStep===4){
      if(!foes.length){
        G.tutStep=5;
        for(let i=0;i<2;i++){ const s=spawnEdge(); if(s) mkFoe(i?"wretch":"hollow",s.x,s.y); }
        if(foes.length<2) mkFoe("wretch",clamp(player.x-9*TILE,TILE,GW*TILE-TILE),player.y);
        say("two more","",[
          "They rarely come alone out here.",
          "<i>Vigor first, then the swing. A winded swing is half a swing.</i>"]);
      }
    } else if(G.tutStep===5){
      if(!foes.length){ G.tutStep=6; startPanCine(); }
    }
  }
  else if(G.tut===2&&G.isl===1){
    if(G.mode!=="play"||interOn) return;
    if(!G.tutR2){ G.tutR2=1;
      say("this beach","",[
        "The fire you crossed for has burned down to a red account of itself, and somebody is face-down beside it.",
        "<i>Stand over them with a bandage and "+(TC.on?"tap E":"press H or E")+". People you help remember it.</i>"]);
      return;
    }
    const got=G.regards.length||party.some(m=>m&&m.g)||(ISL.spirit&&ISL.spirit.tut&&ISL.spirit.done);
    if(!G.tutReg&&got){
      G.tutReg=1; G.tut=3;
      const m=party.find(m2=>m2&&m2.tut)||party[0];
      if(m) say(m.n,m.role,[
        `<i>"So it was true. Good. I'd have felt a liar forever."</i>`,
        `<i>"When my ship went down I came out of the east \u2014 days of it, fog the whole way. Whatever is left of the world is in that direction."</i>`,
        "The raft is on the sand where you left it. East, then."]);
      else toast("east","He came out of the east. Whatever is left of the world is that way.");
    }
  }
  else if(G.tut===4&&G.isl===2){
    if(G.mode!=="play"||interOn||interQ.length) return;
    G.tut=9;
    const m=party.find(m2=>m2&&m2.tut)||party[0];
    say(m?m.n:"a thought",m?m.role:"",[
      m?`<i>"Quiet one, this. If you laid a hearth here the island would keep \u2014 nothing grows back over your work on your own sand, and it stays on the chart."</i>`
       :"A quiet island. A hearth laid here would make it yours \u2014 nothing would grow back over your work, and it would stay on the chart.",
      "<i>Not required. But the sea is long, and it is a good thing to own a door.</i>"]);
  }
}

/* ---- the pan: across real water to a real island ---- */
let CINE_KEEP=null;
function easeIO(x){ return x<.5?2*x*x:1-Math.pow(-2*x+2,2)/2; }
function cineExtend(){
  /* remember the world exactly, widen it east, and lay the SHORE of a larger
     land out in the new water - a coastline running the full height of the
     view with land continuing off-screen, so the island you then sail to
     reads as the same coast */
  CINE_KEEP={gw:GW,gh:GH,grid:grid.slice(),reach:reach.slice()};
  const ow=GW;
  resizeWorld(ow+40,GH);
  for(let y=0;y<GH;y++)for(let x=0;x<ow;x++){
    grid[y*GW+x]=CINE_KEEP.grid[y*CINE_KEEP.gw+x];
    reach[y*GW+x]=CINE_KEEP.reach[y*CINE_KEEP.gw+x];
  }
  const base=ow+24;
  let rs=(base*73+GH*151)>>>0; const rr=()=>{ rs=(rs*1664525+1013904223)>>>0; return rs/4294967296; };
  const coast=[];
  for(let y=0;y<GH;y++)
    coast[y]=Math.round(base+Math.sin(y*.42)*2.2+Math.sin(y*.13+1.7)*1.6);
  for(let y=0;y<GH;y++){
    const cx0=coast[y];
    setT(cx0-1,y,SHOAL);
    for(let x=cx0;x<GW;x++){
      const inland=x-cx0;
      if(inland<2){ setT(x,y,SAND); continue; }
      setT(x,y,GRASS);
      const r2=rr();
      if(inland>=3){
        if(r2<.10) setT(x,y,PINE);
        else if(r2<.17) setT(x,y,TREE);
        else if(r2<.22) setT(x,y,SCRUB);
      }
    }
  }
  /* the fire sits just up from the waterline, on cleared ground */
  const ity=clamp(Math.floor(player.y/TILE),9,GH-10);
  const itx=coast[ity]+3;
  for(let dy=-1;dy<=1;dy++)for(let dx=-2;dx<=2;dx++) setT(itx+dx,ity+dy,GRASS);
  return {ix:itx*TILE+14,iy:ity*TILE+14};
}
function cineRestore(){
  if(!CINE_KEEP) return;
  resizeWorld(CINE_KEEP.gw,CINE_KEEP.gh);
  grid.set(CINE_KEEP.grid); reach.set(CINE_KEEP.reach);
  CINE_KEEP=null;
}
function startPanCine(){
  G.tut=1;
  strandGuard(ISL.start);
  const at=cineExtend();
  G.cine={t:0,ix:at.ix,iy:at.iy,
    x0:(cam.fx===undefined?cam.x:cam.fx),y0:(cam.fy===undefined?cam.y:cam.fy),
    x1:Math.max(0,GW*TILE-VW),
    y1:clamp(at.iy-VH*.52,0,Math.max(0,GH*TILE-VH))};
  G.mode="cine";
}
function tickCine(dt){
  const c=G.cine; if(!c) return;
  c.t+=dt;
  const A=3.6,B=3.4,C=2.6;
  let f;
  if(c.t<A) f=easeIO(c.t/A);
  else if(c.t<A+B){
    f=1;
    if(!c.told){ c.told=1; toast("across the water","A fire. Two of them. One is not standing."); }
  }
  else if(c.t<A+B+C) f=1-easeIO((c.t-A-B)/C);
  else {
    G.cine=null; cineRestore(); G.mode="play";
    interlude("The far fire","Somebody is over there",
      "A fire on a far shore, burning low, and two people by it \u2014 one of them face-down on the sand "+
      "and staying there. You cannot swim it. Nobody swims this water.<br><br>"+
      "<i>The wreck has given the sand enough for a raft. Build it \u2014 the craft panel is "+(TC.on?"CRAFT in the strip":"C")+" \u2014 and go.</i>",
      null);
    syncHUD();
    return;
  }
  cam.fx=c.x0+(c.x1-c.x0)*f;
  cam.fy=c.y0+(c.y1-c.y0)*f;
}
function drawCineScene(){
  const c=G.cine; if(!c) return;
  const t=performance.now()/1000;
  const fx=c.ix-cam.x, fy=c.iy-cam.y;
  if(fx<-360||fx>VW+360) return;
  /* the fire, low and holding */
  const fl=.75+.25*Math.sin(t*9)+.12*Math.sin(t*23);
  const gr=ctx.createRadialGradient(fx,fy,2,fx,fy,44*fl);
  gr.addColorStop(0,"rgba(224,161,65,.45)"); gr.addColorStop(1,"rgba(224,161,65,0)");
  ctx.fillStyle=gr; ctx.beginPath(); ctx.arc(fx,fy,44*fl,0,6.3); ctx.fill();
  ctx.fillStyle="#4a3a2c"; ctx.fillRect(fx-7,fy+1,14,3);
  ctx.fillStyle="#e0a141"; ctx.beginPath(); ctx.arc(fx,fy-1,4+2*fl,0,6.3); ctx.fill();
  ctx.fillStyle="#c25334"; ctx.beginPath(); ctx.arc(fx,fy-4,2.6+1.4*fl,0,6.3); ctx.fill();
  if(rnd()<.5) parts.push({x:c.ix+(rnd()*8-4),y:c.iy-2,vx:(rnd()-.5)*10,vy:-26-rnd()*22,
    life:.9,c:rnd()<.5?"#e0a141":"#8b9793",s:2});
  /* one standing at the fire, watching the treeline */
  figure({x:c.ix-24,y:c.iy+6,r:9,ph:t*1.6,face:.5},null,"#54626b",1,"castaway");
  /* one face-down and staying there */
  const dx=fx+24, dy=fy+10;
  ctx.fillStyle="rgba(10,16,18,.30)"; ctx.beginPath(); ctx.ellipse(dx,dy+4,15,5,0,0,6.3); ctx.fill();
  ctx.fillStyle="#6a5a49"; ctx.beginPath(); ctx.ellipse(dx,dy,13,6,0,0,6.3); ctx.fill();
  ctx.fillStyle="#e6dcc6"; ctx.beginPath(); ctx.arc(dx-12,dy-2,4.5,0,6.3); ctx.fill();
}
/* ============================================================ INPUT */
const K={};
...
function drawWorld(){
  ...
  if(G.cine||GW*TILE<VW||GH*TILE<VH){ ctx.fillStyle="#0a171d"; ctx.fillRect(-10,-10,VW+20,VH+20); }
...
function drawDown(m){
  ...
  if(sprC("down",sx,sy,1)){
    ...
    if(!m.tut){ ctx.font="9px 'Courier New', monospace"; ctx.textAlign="center";
...
function beginGame(){
  ...
  G.diff=diffGet();
  /* the first three islands teach, unless two of you are crossing together */
  G.tut=NET.on?9:0; G.tutStep=0; G.tutW=0; G.tutPack=0; G.tutReg=0; G.tutRuins=0; G.cine=null;
...
function diffSet(d){
  if(!DIFFMUL[d]) d="easy";
  try{ localStorage.setItem("castaway_diff",d); }catch(e){}
  $("diffcur").textContent=d;
  for(const k of ["easy","hard","canon"]) $("df_"+k).classList[k===d?"add":"remove"]("sel");
}
function tsub(id){
  const want=$(id).style.display!=="block";
  for(const s of ["diffbox","setbox","mpbox"]) $(s).style.display="none";
  if(want) $(id).style.display="block";
}
$("diffbtn").onclick=()=>tsub("diffbox");
$("setbtn").onclick=()=>tsub("setbox");
$("mpbtn").onclick=()=>tsub("mpbox");
$("df_easy").onclick=()=>diffSet("easy");
$("df_hard").onclick=()=>diffSet("hard");
$("df_canon").onclick=()=>diffSet("canon");
diffSet(diffGet());
$("sndbtn").textContent="sound \u00b7 "+(SND.off?"off":"on");
$("sndbtn").onclick=()=>{
  SND.off=!SND.off;
  try{ localStorage.setItem("castaway_snd",SND.off?"off":"on"); }catch(e){}
  $("sndbtn").textContent="sound \u00b7 "+(SND.off?"off":"on");
};

/* ---- the title water: an archipelago that was always there ---- */
(function(){
  const c=$("titlebg"); if(!c||!c.getContext) return;
  const g=c.getContext("2d"), W=c.width, H=c.height;
  let ts=20260807; const tr=()=>{ ts=(ts*1664525+1013904223)>>>0; return ts/4294967296; };
  const isles=[];
  for(let i=0;i<11&&isles.length<7;i++){
    const cx=34+tr()*(W-68), cy=28+tr()*(H-56), r=5+tr()*11;
    let ok=true;
    for(const o of isles) if(Math.hypot(o.cx-cx,o.cy-cy)<o.r+r+18) ok=false;
    if(ok) isles.push({cx,cy,r,green:tr()<.62,ph:tr()*6.3});
  }
  const px=3;
  /* the land does not move; draw it once */
  const land=document.createElement("canvas"); land.width=W; land.height=H;
  const lg=land.getContext("2d");
  if(lg) for(const o of isles){
    for(let a=0;a<6.3;a+=.2){
      const wob=1+Math.sin(a*3+o.ph)*.3+Math.sin(a*7+o.ph*2)*.12;
      const rr=o.r*wob;
      for(let d=rr;d>0;d-=1){
        const x=o.cx+Math.cos(a)*d, y=o.cy+Math.sin(a)*d*.76;
        lg.fillStyle=d>rr-1.7?"#8b8064":(o.green?"#2f4a3a":"#5b6165");
        lg.fillRect(Math.round(x/px)*px,Math.round(y/px)*px,px,px);
      }
    }
  }
  function frame(){
    ...
    g.fillStyle="#0a1214"; g.fillRect(0,0,W,H);
    /* water shimmer */
    g.fillStyle="#10242c";
    for(let i=0;i<160;i++){
      const x=(i*97)%W, y=(i*61)%H;
      if(Math.sin(t*.0011+i*1.71)>.88) g.fillRect(x,y,px,1);
    }
    if(lg) g.drawImage(land,0,0);
    /* a slow ring of surf around each coast */
    for(const o of isles){
      const s=.5+.5*Math.sin(t*.0009+o.ph);
      g.strokeStyle="rgba(22,50,58,"+(.3+.35*s).toFixed(2)+")";
      g.lineWidth=1.5;
      g.beginPath(); g.ellipse(o.cx,o.cy,o.r+3+s*2.4,(o.r+3+s*2.4)*.76,0,0,6.3); g.stroke();
    }
    /* fog, in bands, going about its business */
    for(let i=0;i<3;i++){
      const y=((H*.31*i+t*.0042+i*47)%(H+44))-22;
      const gr=g.createLinearGradient(0,y,0,y+18);
      gr.addColorStop(0,"rgba(139,151,147,0)");
      gr.addColorStop(.5,"rgba(139,151,147,.05)");
      gr.addColorStop(1,"rgba(139,151,147,0)");
      g.fillStyle=gr; g.fillRect(0,y,W,18);
    }
  }
  requestAnimationFrame(frame);
})();
...
function saveRun(){
  ...
  if(G.cine) return false;                    // never keep the widened pan world
  if(G.mode==="title"||G.mode==="ending") return;
  try{
    const gg=Object.assign({},G);
    gg.regards=packRegards(G.regards);
    gg.halt=false; gg.hitstop=0; gg.shake=0; gg.hurtT=0; gg.mode="play"; gg.cine=null;
